## NZ Steel free industrial allocation of emissions units 2010 to 2024
...
excel_sheets("Industrial-Allocations-Final-Decisions-Report-7-Oct-2025.xlsx")
[1] "IA Final Decisions"
# read in allocation of emissions units data
Allocations <- read_excel("Industrial-Allocations-Final-Decisions-Report-7-Oct-2025.xlsx", sheet = "IA Final Decisions",skip=3)
# read in allocation of emissions units data
...
str(Allocations) 
tibble [1,585 × 4] (S3: tbl_df/tbl/data.frame)
 $ Activity        : chr [1:1585] "Fresh tomatoes" "Protein meal" "Protein meal" "Fresh cucumbers" ...
 $ Applicant’s name: chr [1:1585] "A1 TOMS LIMITED" "Affco New Zealand Limited" "Alliance Group Limited" "B.S.K. Growers Limited" ...
 $ Year            : num [1:1585] 2024 2024 2024 2024 2024 ...
 $ Final Allocation: num [1:1585] 297 13615 12151 102 315101 ...
...
summary(Allocations)   
 ...
 Length:1585        Length:1585        Min.   :2010   Min.   :      1
 Class :character   Class :character   1st Qu.:2012   1st Qu.:    194
 Mode  :character   Mode  :character   Median :2016   Median :   1178
                                       Mean   :2016   Mean   :  49516
                                       3rd Qu.:2020   3rd Qu.:   8250
                                       Max.   :2024   Max.   :2145482
...
nzu2024 <- filter(Allocations, Year =="2024")
...
nzu2024[["MeanMayprice"]] <- 51.39
nzu2024[["Value"]] <- nzu2023[["Allocation"]]* 51.39

str(nzu2024)

# combine all the year data together into 1 dataframe - I use rbind as all the column names are consistent
Allocations <- rbind(nzu2010,nzu2011,nzu2012,nzu2013,nzu2014,nzu2015,nzu2016,nzu2017,nzu2018,nzu2019,nzu2020,nzu2021,nzu2022,nzu2023,nzu2024)

# check the new dataframe
str(Allocations)
tibble [1,585 × 6] (S3: tbl_df/tbl/data.frame)
 $ Activity    : chr [1:1585] "Glass containers" "Protein meal" "Protein meal" "Fresh cucumbers" ...
 $ Applicant   : chr [1:1585] "ACI OPERATIONS NZ LIMITED" "Affco New Zealand Limited" "Alliance Group Limited" "Alwyn Ernest Inger, Anne Marie Inger" ...
 $ Year        : num [1:1585] 2010 2010 2010 2010 2010 2010 2010 2010 2010 2010 ...
 $ Allocation  : num [1:1585] 14454 2798 2119 168 3654 ...
 $ MeanMayprice: num [1:1585] 17.6 17.6 17.6 17.6 17.6 ...
 $ Value       : num [1:1585] 254101 49189 37252 2953 64237 ...
...
15 Iron and steel manufacturing … New Zeal…  2024    1594941        51.4  2.39e5
...
tibble [15 × 6] (S3: tbl_df/tbl/data.frame)
 $ Activity    : chr [1:15] "Iron and steel manufacturing from iron sand" "Iron and steel manufacturing from iron sand" "Iron and steel manufacturing from iron sand" "Iron and steel manufacturing from iron sand" ...
 $ Applicant   : chr [1:15] "New Zealand Steel Development Limited" "New Zealand Steel Development Limited" "New Zealand Steel Development Limited" "New Zealand Steel Development Limited" ...
...
excel_sheets("GHG-Inventory-2026-Time-series-emissions-data-1990-to-2024.xlsx")
[1] "All gases" "CO2"       "CH4"       "N2O"       "HFCs"      "PFCs"     
[7] "SF6"

# read in inventory emissions data
emissions <- read_excel("GHG-Inventory-2026-Time-series-emissions-data-1990-to-2024.xlsx")

# filter out only steel sector emissions being row 376 from column 3 1990 to 2022 2.C.1. Iron and steel product/2.C.1. Iron and steel production
emissions[376,3:35]

emissions[376,36:37]

steel <- as.numeric(emissions[376,3:37])

# check steel vector
str(steel)
num [1:35] 1307 1421 1499 1535 1417 ...
# print to screen
steel
...
steel <-c(steel * 10^3)
str(steel)
 num [1:35] 1306735 1421220 1498642 1535022 1416737 ...
# select only 2010 to 2024 emissions
steel <- steel[21:35]
steel
...
## obtain ETS Participant emissions report data from EPA www
# https://www.epa.govt.nz/industry-areas/emissions-trading-scheme/ets-reports/emissions-returns/
etssteel <- structure(list(Year = c(2020, 2021, 2022, 2023), Emissions = c(1553344, 1889288, 1481654, 1359082)), row.names = c(NA, -4L), class = "data.frame")
...
unitdiscount <- c(0.25,0.5,0.5,0.5,0.5,0.5,0.5,0.67,0.83,1,1,1,1,1,1)
str(unitdiscount)

num [1:15] 0.25 0.5 0.5 0.5 0.5 0.5 0.5 0.67 0.83 1
...
[1] 3.885827 # [1] 3.953194 # 3.113329 # almost 4 million units
...
ERU <- c(rep(NA,3) ,c(1022527,1001714,32534) , rep(NA,9))
str(ERU)
 num [1:15] NA NA NA 1022527 1001714
...
NZsteelunits[["Stockpile"]][15] <- NZsteelunits[["Stockpile"]][14]+NZsteelunits[["Overallocation"]][15]

# How big is the estimated stockpile after the 2024 calendar year
NZsteelunits[["Stockpile"]][15]
[1] 5942602
# [1] 6009969
...
# What is the market value at the most recent NZU price? and what is market value of NZ Steel stockpile ?
2026-06-19,54 emsTradepoint
round(NZsteelunits[["Stockpile"]][15] * 54,2)
[1] 320900508
# $320 million
...
New Zealand Steel Development Limited	Industrial Processes	Producing iron or steel	Three	2024	 38734
New Zealand Steel Limited	Stationary Energy	Importing coal	Three	2024	 410116
New Zealand Steel Limited	Stationary Energy	Purchasing coal	Four	2024	 1084350
...
axis(side=1, tck=0.01, las=0, lwd = 1, at = c(2010:2024), labels = c(2010:2024), tick = TRUE)
...
legend(2011, 1.2, bty = "n",cex=1.1, "Steel emissions MfE Greenhouse Gas Inventory 2024",col = "#1B9E77", text.col = 1,lty = 1, pch=16 )
legend(2011, 1.0, bty = "n",cex=1.1, "NZ Steel emissions EPA Participant Reports 2020 2024", col="red",text.col=1, pch=19)
mtext(side=3,cex=1.5, line=-3.5, expression(paste("New Zealand Steel Limited greenhouse gas emissions \n2010 to 2024")) )
mtext(side=1,line=-1.25,cex=1,"Data: New Zealand’s Greenhouse Gas Inventory 1990 – 2024\nETS Participant Emissions EPA 2020 2021 2024")
...
points(2024,sum(38734,410116,1084350)/10^6,cex=1.5,pch=19, col="red")
dev.off() 
# add ETS emissions
#lines(etssteel[["Year"]],etssteel[["Emissions"]]/10^6,col="red",lwd=1)
#points(etssteel[["Year"]],etssteel[["Emissions"]]/10^6,cex=1,pch=16, col="red")
...
mtext(side=3,cex=1.5, line=-3.5, expression(paste("New Zealand Steel Limited greenhouse gas emissions and\nemission unit allocation 2010 to 2024")))
mtext(side=1,line=-1.25,cex=1,"Data: New Zealand’s Greenhouse Gas Inventory 1990 – 2024, EPA Industrial Allocations")
mtext(side=2,cex=1, line=-1.2,expression(paste("million tonnes C", O[2], "-e")))
mtext(side=4,cex=0.75, line=0.05,R.version.string)
box()
dev.off() 
...
mtext(side=3,cex=1.5,line=-3.5,expression(paste("New Zealand Steel Limited overallocation of emissions units\n above ETS liability 2010 to 2024")))
mtext(side=1,line=-1.25,cex=1,"Data: New Zealand’s Greenhouse Gas Inventory 1990 2024, EPA Industrial Allocations 2024")
mtext(side=2,cex=1, line=-1.2,expression(paste("million tonnes C", O[2], "-e")))
mtext(side=4,cex=0.75, line=0.05,R.version.string)
box()
dev.off()
...
points(NZsteelunits[["Year"]],NZsteelunits[["ERU"]]/10^6,col="#66A61E",cex=1.25,pch=16)
lines(NZsteelunits[["Year"]],NZsteelunits[["ERU"]]/10^6,col="#66A61E",cex=1.25,lwd=1)
legend(2010, 5.25, cex=1, bty = "n", c("Actual Emissions","Emission units allocated","Emissions x 'two for one' equals ETS liability","Emission Reduction Units add to surplus units","Allocation less ETS liability = annual surplus of units","surplus + ERUs =  stockpile of ~ 6 million units 2024?"), col=c("#1B9E77","#D95F02","#7570B3","#66A61E","#E7298A","#A6761D") , text.col = 1, lty = 1, pch = c(16,17,15,16,18,19))
mtext(side=3,cex=1.5,line=-3.5, expression(paste("NZ Steel Limited cumulative stockpile of \nemissions units 2010 2024")) )
mtext(side=1,line=-1.25,cex=1,"Data: NZ Greenhouse Gas Inventory 2024, EPA Industrial Allocations 2024")
mtext(side=2,cex=1, line=-1.2,expression(paste("million tonnes C", O[2], "-e")))
mtext(side=4,cex=0.75, line=0.05,R.version.string)
box()
dev.off()
...
# what is the total mass of emissions pemitted by free emission units?  30 million
sum(NZsteelunits[["Footprint"]]) 
[1] 30317358
# [1] 28722417
# [1] 24981914
# what is the total Inventory steel sector emissions?       25 m
sum(NZsteelunits[["Emissions"]])
[1] 24941540
# [1] 23279154
# [1] 20378517 
# what are the total steel sector allocation of emissions?      22 m
...
mtext(side=3,cex=1.5,line=-3.5, expression(paste("New Zealand Steel Limited emissions footprint of \nfree allocation of emissions units 2010 to 2024")) )
mtext(side=3,cex=1.1,line=-6, expression(paste("The 22m free units were equivalent to an emissions footprint \nof 30m tonnes when actual steel emissions were 25m tonnes")) )
mtext(side=1,line=-1.25,cex=1,"Data: NZ Greenhouse Gas Inventory 2024, EPA Industrial Allocations 2024")
mtext(side=2,cex=1, line=-1.2,expression(paste("million tonnes C", O[2], "-e")))
mtext(side=4,cex=0.75, line=0.05,R.version.string)
box()
